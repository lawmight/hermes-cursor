...
The cursor-sdk import is lazy (optional extra ``hermes-cursor[sdk]``,
pinned ``cursor-sdk==1.0.27``) and injectable (``sdk_module=``) so tests
never touch the ~48 MB wheel or the network.
"""
...
# TODO(sdk-1.x): retest Agent.create/resume/cancel + bridge launch against
# cursor-sdk==1.0.27; update option builders if LocalAgentOptions /
# CloudAgentOptions shapes changed. This module is kept for a future
# external-harness seam — MVP ships CLI + skill only (no ProviderProfile).

from hermes_cursor.runtime.bridge import launch_cursor_bridge
from hermes_cursor.runtime.event_projector import CursorEventProjector


def redact_sensitive_text(text: str) -> str:
    """Best-effort redaction; prefer Hermes' helper when available."""
    try:
        from agent.redact import redact_sensitive_text as _hermes_redact  # type: ignore

        return _hermes_redact(text)
    except Exception:
        return text
...
def get_hermes_home() -> Path:
    try:
        from hermes_constants import get_hermes_home as _gh  # type: ignore
...
def atomic_json_write(path: Path, data: Any) -> None:
    """Write JSON atomically; use Hermes util when present."""
    try:
        from utils import atomic_json_write as _ajw  # type: ignore
    ...
    fd, tmp = tempfile.mkstemp(dir=str(path.parent), prefix=".cursor-sessions.")
    try:
...
class CursorSDKSession:
    ...
    def _import_sdk(self) -> Any:
        if self._sdk is not None:
            return self._sdk
        try:
            import cursor_sdk  # noqa: PLC0415 — lazy (optional extra)
        except ImportError as exc:
            raise RuntimeError(
                "cursor-sdk is not installed. Install the plugin SDK extra:\n"
                "  pip install 'hermes-cursor[sdk]'\n"
                "or:  pip install 'cursor-sdk==1.0.27'"
            ) from exc
    ...
    def _build_local_options(self) -> dict[str, Any]:
        ...
        # MVP / #70140: Cursor owns tools inside the SDK run. Hermes custom
        # tool bridging (inventory cursor_hermes_tools) is deferred — only
        # honor an explicit injectable builder for tests / future seams.
        if self._config.get("expose_hermes_tools", False):
            builder = self._custom_tools_builder
            if builder is None:
                logger.debug(
                    "expose_hermes_tools requested but no custom_tools_builder "
                    "was injected; hermes_tools bridge is deferred for MVP"
                )
                custom_tools = {}
            else:
                try:
                    ...
                    logger.exception(
                        "cursor custom_tools build failed; continuing without"
                    )
                    custom_tools = {}
            if custom_tools:
