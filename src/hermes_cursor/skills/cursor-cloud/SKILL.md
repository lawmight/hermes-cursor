...
version: 0.1.0
author: lawmight / Tom Coustols (community plugin)
license: MIT
metadata:
  hermes:
...
**Honest scope:** this skill + CLI only. Cursor owns the inner agent loop /
tools / sandbox. Hermes keeps sessions, memory, gateway, and cron when you
drive via this skill. This plugin does **not** provide
`model.provider=cursor` / `hermes model` → Cursor as a primary chat provider.
...
- For local Hermes-native subagents prefer `delegate_task`.

## Prerequisites

- `CURSOR_API_KEY` in the Hermes `.env` (Cursor Dashboard → Integrations →
  API Keys). Verify with `terminal`: `hermes cursor me`.
- Optional SDK extra installed for agent verbs:
  `pip install 'hermes-cursor[sdk]'` (pins `cursor-sdk==1.0.27`).
  REST verbs `me` / `models` work without the SDK.
